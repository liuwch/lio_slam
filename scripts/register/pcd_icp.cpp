  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (fname.compare (fname.size () - extension.size (), extension.size (), extension) == 0)
    {
      ...
      if (i == 1) {
        ...
        passthrough.setInputCloud(m.cloud);//输入点云
        passthrough.setFilterFieldName("z");//对z轴进行操作
        passthrough.setFilterLimits(0.5, 4);//设置直通滤波器操作范围
        ...
        passthrough_y.setInputCloud(m.cloud);//输入点云
        passthrough_y.setFilterFieldName("y");//对z轴进行操作
        passthrough_y.setFilterLimits(-3, 20);//设置直通滤波器操作范围
        // passthrough.setFilterLimitsNegative(true);//true表示保留范围内，false表示保留范围外
        passthrough_y.filter(*m.cloud);

        pcl::PassThrough<pcl::PointXYZ> passthrough_x;
        passthrough_x.setInputCloud(m.cloud);//输入点云
        passthrough_x.setFilterFieldName("x");//对z轴进行操作
        passthrough_x.setFilterLimits(-50, 50);//设置直通滤波器操作范围
        // passthrough.setFilterLimitsNegative(true);//true表示保留范围内，false表示保留范围外
      ...
      if (i == 2) {
        ...
        passthrough_y.setInputCloud(m.cloud);//输入点云
        passthrough_y.setFilterFieldName("y");//对z轴进行操作
        passthrough_y.setFilterLimits(-20, 10);//设置直通滤波器操作范围
        // passthrough.setFilterLimitsNegative(true);//true表示保留范围内，false表示保留范围外
        passthrough_y.filter(*m.cloud);
