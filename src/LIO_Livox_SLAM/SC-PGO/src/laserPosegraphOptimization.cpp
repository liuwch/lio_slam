...
void loopFindNearKeyframesCloud( pcl::PointCloud<PointType>::Ptr& nearKeyframes, pcl::PointCloud<PointType>::Ptr& nearKeyframesViz, const int& key, const int& submap_size)
{
    ...
    for (int i = -submap_size; i <= submap_size; ++i) {
        ...
        *nearKeyframes += *local2global(keyframeLaserClouds[keyNear], pose_relative);                       // 以当前帧位置为原点
        *nearKeyframesViz += *local2global(keyframeLaserClouds[keyNear], keyframePosesUpdated[keyNear]);    // 以初始位置为原点
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    pcl::PointCloud<PointType>::Ptr currKeyframeCloud(new pcl::PointCloud<PointType>());            // 以当前位置为坐标原点
    pcl::PointCloud<PointType>::Ptr currKeyframeCloudViz(new pcl::PointCloud<PointType>());         // 以初始位置为坐标原点
    ...
    loopFindNearKeyframesCloud(targetKeyframeCloud, targetKeyframeCloudViz, _loop_kf_idx, historyKeyframeSearchNum);   //  对历史帧中的前后25帧进行融合
    ...
    // Align pointclouds
    // target不变，source经过变化使两点集重合
    icp.setInputSource(currKeyframeCloud);
    icp.setInputTarget(targetKeyframeCloud);
    pcl::PointCloud<PointType>::Ptr unused_result(new pcl::PointCloud<PointType>());
    icp.align(*unused_result, T_init.matrix().cast<float>());   // 输出配准后点云（将当前帧配准到历史帧中），T_init初始位姿
    ...
    *icpKeyframeCloudViz += *local2global(unused_result, pose_loop);
    ...
    correctionLidarFrame = icp.getFinalTransformation();    // 获取最终的转换矩阵
    ...
    return poseFrom.between(poseTo);  // 得到的是poseTo相对于poseFrom的姿态变化，即以poseFrom为基准坐标，poseTo在poseFrom基准坐标下的坐标。本函数返回的是历史帧与当前帧之间的残差
		...
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    // }
                    initialEstimate.insert(curr_node_idx, poseTo);      // 添加对状态值的初始约束          
                    // runISAM2opt();
                }
                mtxPosegraph.unlock();
...
void performSCLoopClosure(void)
{
    ...
    std::cout << "keyframePoses: " << keyframePoses.size() << std::endl;
...
void process_icp(void)
{
    while(1)
    {
        ...
        {
            std::cout << "scLoopICPBuf: " << scLoopICPBuf.size() << std::endl;
...
void process_interactive_slam_result() {
    ros::Rate rate(50);
    ROS_INFO("process_interactive_slam_result!");
    mKF.lock();
    std::cout << "keyframePosesUpdated: " << keyframePosesUpdated.size() << std::endl;
    for (int node_idx=0; node_idx < int(keyframePosesUpdated.size()); node_idx++) {
        std::cout << "interactive slam node idx: " << node_idx << std::endl;
...
void save_map() {
    ...
    laserCloudMapPGO->clear();
    for (int node_idx=0; node_idx < int(keyframePosesUpdated.size()); node_idx++) {
        std::cout << "save_map node idx: " << node_idx << std::endl;
        *laserCloudMapPGO += *local2global(keyframeLaserClouds[node_idx], keyframePosesUpdated[node_idx]);
    }
...
int main(int argc, char **argv)
{
    ...
    std::cout << "save_odo_path = " << save_path << std::endl;
