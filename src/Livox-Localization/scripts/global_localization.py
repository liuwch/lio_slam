...
def registration_at_scale(pc_scan, pc_map, initial, scale):
    result_icp = o3d.registration.registration_icp(
        voxel_down_sample(pc_scan, SCAN_VOXEL_SIZE *
                          scale), voxel_down_sample(pc_map, MAP_VOXEL_SIZE * scale),
        1.0 * scale, initial,
...
def crop_global_map_in_FOV(global_map, pose_estimation, cur_odom):
    ...
    global_map_in_base_link = np.matmul(
        T_base_link_to_map, global_map_in_map.T).T
    ...
    if FOV > 3.14:
        ...
        indices = np.where(
            ...
            (np.abs(np.arctan2(
                global_map_in_base_link[:, 1], global_map_in_base_link[:, 0])) < FOV / 2.0)
        )
    else:
        # 非环状lidar 保前视范围
    ...
    header.frame_id = 'map'
    publish_point_cloud(pub_submap, header, np.array(
        global_map_in_FOV.points)[::10])
...
def global_localization(pose_estimation):
    ...
    global_map_in_FOV = crop_global_map_in_FOV(
        global_map, pose_estimation, cur_odom)
...
if __name__ == '__main__':
    ...
    pub_pc_in_map = rospy.Publisher(
        '/cur_scan_in_map', PointCloud2, queue_size=1)
    ...
    file_path = "/liuwch/results/around_park"
